-- 016 — `about_the_artwork` como campo propio y `value_text` en cada detalle.
--
-- Cierra el hueco que reportó el consumidor: el texto sobre la obra viajaba
-- como árbol JSON de Shopify dentro de `details[]` y no había forma de
-- mostrarlo. Ambos cambios son ADITIVOS; `value` se conserva crudo.
--
--   · `details[].value_text` — siempre legible. Para rich_text_field es el
--     árbol aplanado (015); para el resto, el mismo `value`. Regla simple
--     para el consumidor: leer value_text y olvidarse del `type`.
--   · `about_the_artwork` — campo del producto, ya en texto plano, siguiendo
--     la convención custom.<clave> -> <clave> de la Decisión 7b.
--   · `shipping` — misma convención, para la nota de envío del artista.
